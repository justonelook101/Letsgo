...
def video_page(vid_id):
    ...
    player_url = request.url_root + f'player/{vid_id}'
    image_url = request.url_root + f'static/images/{vid_id}.{image_ext}'
    
    # Hardcoded values - adjust as needed
    twitter_site = '@yourusername'  # Replace with your X/Twitter username
    title = f'Video {vid_id}'
    description = 'An uploaded video'
    width = 640
    height = 360  # Assuming 16:9 aspect ratio
    ...
    <body>
        <h1>{title}</h1>
        <p>Share this link on X/Twitter to see the Player Card.</p>
